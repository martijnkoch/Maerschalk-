<template>
  <div class="container-fluid header-container">
      ...
      <b-container class="bv-example-row mt-5">
       <div class="d-flex noodoproep-title">
        <div class="py-2 pr-2"><h2>{{call.company_name}}</h2></div>
        <div class="mr-auto p-2 mt-1"><h4>{{call.phone }}</h4></div>
        <div class="p-2 mt-1"><h4>{{call.phone}}</h4></div>
        <div class="py-2 pl-2"><b-button class="homepage-cardbutton mt-1" variant="warning" :class="[call.status.toLowerCase()]">{{call.status}}</b-button></div>
        </div>
        ...
        <b-button class="mr-3 text-white" variant="warning" v-on:click="updateCall(call.id)">Update</b-button>
